...
{
 /* ROSE-UE64: chunks are authored by the 32-bit tool; size_t on disk is
    always 4 bytes regardless of host pointer width. */
 unsigned int x;
 LoadBlock(L,&x,sizeof(x),Z,swap);
 return (size_t)x;
}
...
static int LoadHeader (lua_State* L, ZIO* Z)
{
 ...
 TESTSIZE(sizeof(int));
 TestSize(L,4,"sizeof(size_t)",Z);	/* ROSE-UE64: on-disk size_t is 4 */
